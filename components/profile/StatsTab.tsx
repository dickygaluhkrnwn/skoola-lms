...
export function StatsTab({ 
  ...
  return (
    <div className={cn(
      ...
      isUni ? "bg-white/5 border-white/10 backdrop-blur-xl shadow-lg" : 
      isSMP ? "bg-white/60 border-white/60 backdrop-blur-md" : 
      isSMA ? "bg-white/5 border-white/10 backdrop-blur-md" : 
      "bg-white border-slate-200"
    )}>
       <h3 className={cn("font-bold text-lg mb-4 flex items-center gap-2", textPrimary)}>
          <Activity size={20} className={cn(isSMA ? "text-teal-500" : isUni ? "text-indigo-400" : "text-blue-500")}/> 
          Aktivitas Belajar
       </h3>
                ...
                <div 
                   ...
                   className={cn(
                      ...
                      (
                        isSMP ? "bg-violet-500" : 
                        isSMA ? "bg-teal-500" : 
                        isUni ? "bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.6)]" : 
                        "bg-green-500"
                      )
                   )}
                   title={`Hari ke-${i+1}`}
       ...
       <div className="flex justify-between text-xs text-gray-400 mt-2">
          ...
          <div className="flex items-center gap-1">
             ...
             <div className={cn("w-2 h-2 rounded-sm", isUni ? "bg-indigo-900" : isSMA ? "bg-teal-800" : "bg-green-300")} />
             <div className={cn("w-2 h-2 rounded-sm", isUni ? "bg-indigo-500" : isSMA ? "bg-teal-500" : "bg-green-500")} />
             <span>Aktif</span>
          </div>
       </div>

       <div className={cn("mt-8 pt-6 border-t border-dashed grid grid-cols-2 gap-4", (isUni || isSMA) ? "border-white/10" : "border-gray-200")}>
          <div>
