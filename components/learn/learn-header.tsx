...
export function LearnHeader({ theme, userProfile, onOpenJoinModal }: LearnHeaderProps) {
  return (
    <header className={cn(
      ...
      theme === "kids" ? "bg-white/90 border-b-4 border-sky-100" : "bg-white/90 border-zinc-200"
    )}>
      <div>
        <h1 className={cn(
          "text-lg font-bold leading-tight",
          theme === "kids" ? "text-sky-900 font-display" : "text-zinc-900"
        )}>
          {theme === "kids" ? "Peta Petualangan 🗺️" : "Dashboard Pembelajaran"}
        </h1>
        <p className={cn("text-xs hidden md:block", theme === "kids" ? "text-sky-500 font-bold" : "text-zinc-500")}>
          {theme === "kids" ? "Ayo kumpulkan semua bintang!" : "Lanjutkan progres kurikulum Anda."}
        </p>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className={cn(
          ...
          <Star size={16} className={cn("fill-current animate-pulse", theme === "kids" ? "text-yellow-500" : "text-indigo-500")} />
        ...
        <div className={cn(
          ...
          <Flame size={16} className={cn("fill-current", theme === "kids" ? "text-orange-500 animate-bounce" : "text-red-500")} />
