...
import { Users, Heart, MessageCircle, Crown, Medal, Trophy } from "lucide-react";
...
import { UserSummary } from "@/lib/types/user.types"; // Menggunakan tipe data standar
...
export default function SocialClient() {
  ...
  const [leaderboardData, setLeaderboardData] = useState<UserSummary[]>([]);
  const [loading, setLoading] = useState(true);
  const [currentUserData, setCurrentUserData] = useState<UserSummary | null>(null);

  // FETCH DATA (Client Side Fetching untuk Realtime Updates nanti)
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 1. Ambil Top 50 User berdasarkan XP
        const q = query(
          collection(db, "users"), 
          where("role", "==", "student"),
          orderBy("xp", "desc"), 
        ...
        querySnapshot.forEach((doc) => {
          const data = doc.data();
          // Mapping data Firestore ke tipe UserSummary
          users.push({
            uid: data.uid,
        ...
        <header className={cn(
          ...
          <div className={cn(
            ...
            theme === "kids" ? "bg-sky-100 border-white" : "bg-secondary border-border"
             ...
             <Trophy size={20} className={cn(theme === "kids" ? "text-yellow-500" : "text-primary")} />
          </div>
        </header>
        ...
        <div className="max-w-md mx-auto px-4 pt-6">
          ...
          {activeTab === "leaderboard" && (
            <div className="space-y-4">
               {loading ? (
                 <div className="text-center py-10 text-gray-400 animate-pulse">Memuat data juara...</div>
                 ...
                 <div className="space-y-3">
                   {leaderboardData.map((user, idx) => {
                     ...
                     return (
                       <motion.div 
                         key={user.uid} // Gunakan UID sebagai key unik
                       ...
                       >
                         ...
                         <div className={cn(
                           "w-10 h-10 rounded-full flex items-center justify-center text-xl border overflow-hidden",
