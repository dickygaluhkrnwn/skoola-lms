...
import ScheduleManagementView from "@/components/admin-school/ScheduleManagementView"; // Import Baru
...
export default function AdminSchoolClient() {
  ...
  // Tambahkan 'schedule' ke activeTab state
  const [activeTab, setActiveTab] = useState<"dashboard" | "users" | "forum" | "settings" | "schedule">("dashboard");
      ...
      try {
        ...
        if (userDoc.exists()) {
          ...
          if (userData.role !== "admin") {
            router.push("/learn"); // Redirect unauthorized user
            return;
          }
    ...
    if (activeTab === "dashboard" && userProfile) {
      const fetchStats = async () => {
        ...
        try {
          ...
          const classesColl = collection(db, "classrooms");
          const classesSnap = await getCountFromServer(classesColl);

          const forumsColl = collection(db, "forums");
          const forumsSnap = await getCountFromServer(forumsColl);
