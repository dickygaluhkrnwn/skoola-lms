...
// --- COMPONENTS ---
const InputField = ({ label, icon, type = "text", value, onChange, placeholder, min, max, isSMA }: any) => {
  const [showPassword, setShowPassword] = useState(false);
  const isPassword = type === "password";

  return (
    <div className="space-y-1.5">
      <label className={cn("text-xs font-bold uppercase tracking-wider ml-1", isSMA ? "text-slate-400" : "text-slate-500")}>{label}</label>
      <div className="relative group">
        <div className={cn("absolute left-4 top-1/2 -translate-y-1/2 transition-colors", isSMA ? "text-slate-500 group-focus-within:text-teal-400" : "text-slate-400 group-focus-within:text-violet-600")}>
          {icon}
        </div>
        <input
        ...
        {isPassword && (
          <button
            ...
            className={cn(
              ...
              isSMA ? "text-slate-500 hover:text-slate-300 hover:bg-white/5" : "text-slate-400 hover:text-slate-600 hover:bg-slate-100"
            )}
          >
...
export default function AuthClient() {
  ...
  // Helper State for SMA Theme Logic
  ...
  if (loading) return (
    <div className={cn("min-h-screen flex items-center justify-center", isSMA ? "bg-slate-950 text-white" : "bg-slate-50")}>
        <Loader2 className={cn("w-10 h-10 animate-spin", isSMA ? "text-teal-500" : "text-violet-600")} />
    </div>
  );
  ...
  return (
    <div className={cn(
      ...
      isSMA ? "bg-slate-950" : "bg-slate-50"
    )}>
      ...
      <div className={cn(
         ...
         isSMA ? "bg-slate-900/40 backdrop-blur-2xl border border-white/10" : "bg-white/60 backdrop-blur-xl border border-white/50"
      )}>
        
        {/* LEFT: VISUAL PANEL */}
        <div className={cn(
           "hidden md:flex w-1/2 relative p-12 flex-col text-white justify-between overflow-hidden transition-colors duration-500",
           isSMA ? "bg-gradient-to-br from-slate-900 to-indigo-950" : "bg-gradient-to-br from-violet-600 to-indigo-700"
        )}>
            {/* Pattern Overlay */}
            ...
            <div className="relative z-10">
               <div className="flex items-center gap-2 mb-6">
                  <div className={cn("p-2 rounded-lg backdrop-blur-sm", isSMA ? "bg-teal-500/20" : "bg-white/20")}>
                     <Sparkles size={24} className={isSMA ? "text-teal-300" : "text-yellow-300"} />
                  </div>
               ...
               <p className={cn("text-lg max-w-sm leading-relaxed", isSMA ? "text-slate-400" : "text-indigo-100/80")}>
                  {isSMA ? "Platform digital terintegrasi untuk produktivitas akademik maksimal." : "Platform sekolah digital dengan gamifikasi yang bikin kamu ketagihan belajar."}
               </p>
            </div>
        ...
        <div className={cn(
           ...
           isSMA ? "bg-slate-950/50 backdrop-blur-md" : "bg-white/50 backdrop-blur-md"
        )}>
            <div className="max-w-sm w-full mx-auto">
                <div className="text-center mb-8">
                   <h2 className={cn("text-2xl font-bold mb-1", isSMA ? "text-white" : "text-slate-800")}>
                ...
                {/* Tabs */}
                <div className={cn("flex p-1 rounded-xl mb-6", isSMA ? "bg-slate-800/50 border border-slate-700" : "bg-slate-100")}>
                   <button 
                      onClick={() => { setView("login"); setError(""); }}
                      className={cn(
                         "flex-1 py-2 text-sm font-bold rounded-lg transition-all",
                         view === "login" 
                           ? (isSMA ? "bg-slate-700 text-white shadow-sm" : "bg-white text-slate-900 shadow-sm") 
                           : (isSMA ? "text-slate-400 hover:text-slate-200" : "text-slate-500 hover:text-slate-700")
                      )}
                   >
                      Masuk
                   </button>
                   ...
                   >
                      {view === "register" && (
                         <div className="space-y-4 animate-in slide-in-from-right-4 fade-in duration-300">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                               <div 
                                  ...
                                  className={cn(
                                     ...
                                     role === "student" 
                                       ? (isSMA ? "border-teal-600 bg-teal-900/30 text-teal-400" : "border-violet-500 bg-violet-50 text-violet-700")
                                       : (isSMA ? "border-slate-700 bg-slate-800/50 text-slate-400 hover:border-slate-600" : "border-slate-200 hover:border-slate-300")
                                  )}
                               >
                            ...
                            {/* Level Selection */}
                            <div>
                               <label className={cn("text-xs font-bold uppercase ml-1 mb-1 block", isSMA ? "text-slate-400" : "text-slate-500")}>Jenjang</label>
                                  ...
                                  {[
                                     ...
                                     <div
                                        ...
                                        className={cn(
                                           ...
                                           schoolLevel === lvl.id 
                                              ? (isSMA ? `border-teal-500 bg-teal-900/30 text-teal-400` : `border-${lvl.color}-500 bg-${lvl.color}-50 text-${lvl.color}-700`) 
                                              : (isSMA ? "border-slate-700 text-slate-400 hover:border-slate-600" : "border-slate-200 hover:border-slate-300")
                                        )}
                                        // Override style dynamic untuk selain SMA agar warna asli tetap muncul
                                        style={schoolLevel === lvl.id && !isSMA ? { borderColor: `var(--color-primary)`, color: `var(--color-primary)`, backgroundColor: `var(--color-secondary)` } : {}}
                                     >
                                        {lvl.label}
                                     </div>
                                  ))}
                      ...
                      {error && (
                         <div className={cn(
                            ...
                            isSMA ? "bg-red-900/20 text-red-400 border-red-900/50" : "bg-red-50 text-red-600 border-red-100"
                         )}>
