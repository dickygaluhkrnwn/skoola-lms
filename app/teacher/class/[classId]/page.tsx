...
import { 
  ArrowLeft, Users, BookOpen, Copy, Plus, MoreVertical, 
  Video, FileText, Trash2, Link as LinkIcon, Loader2 
} from "lucide-react";
import { auth, db } from "@/lib/firebase";
import { 
  doc, getDoc, collection, addDoc, query, orderBy, onSnapshot, deleteDoc, serverTimestamp 
...
export default function ClassDetailPage() {
  ...
  const [matContent, setMatContent] = useState(""); // Link YouTube atau Teks Materi

  // 1. FETCH DATA KELAS & LISTENER MATERI
  useEffect(() => {
    if (!classId) return;
    ...
    const fetchClassInfo = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          // Fetch Students Data (Manual fetch user profiles based on ID array)
  ...
  const handleAddMaterial = async (e: React.FormEvent) => {
    ...
    try {
      await addDoc(collection(db, "classrooms", classId, "materials"), {
        ...
        type: matType,
        content: matContent,
        createdAt: serverTimestamp(), // PENTING: Timestamp server
      });
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-blue-50 text-blue-600"><Loader2 className="animate-spin mr-2"/> Memuat Kelas...</div>;
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex gap-1 bg-white p-1 rounded-xl shadow-sm border border-slate-200 mb-6 w-fit">
          <button 
            ...
            className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === "students" ? "bg-blue-100 text-blue-700" : "text-slate-500 hover:bg-slate-50"}`}
          >
            Daftar Murid
          </button>
          <button 
            onClick={() => setActiveTab("materials")}
            className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === "materials" ? "bg-blue-100 text-blue-700" : "text-slate-500 hover:bg-slate-50"}`}
          >
            Materi Belajar
          </button>
        </div>
