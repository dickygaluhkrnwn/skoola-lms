...
import { 
  Plus, Users, BookOpen, Settings, LogOut, Copy, Check, School
} from "lucide-react";
...
export default function TeacherDashboard() {
    ...
    const init = async () => {
      ...
      if (userDoc.exists()) {
        ...
        if (userData.role !== "teacher") {
          router.push("/learn"); // Redirect jika bukan guru
          return;
        }
      ...
      // Query: cari di collection 'classrooms' yang 'teacherId' == user.uid
      try {
  ...
  const generateClassCode = () => {
    // Generate kode acak 6 karakter (Huruf + Angka)
  ...
  const handleCreateClass = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update State Lokal (biar langsung muncul tanpa refresh)
      setClassrooms([...classrooms, { id: docRef.id, ...newClass }]);
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-blue-50 text-blue-600">Loading Ruang Guru...</div>;
